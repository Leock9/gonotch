...
var table = map[string][2]string{ // key → {pt, en}
	...
	"update_to":       {"Atualizar para %s", "Update to %s"},
	"whats_new":       {"Novidades da %s", "What's new in %s"},
	"update_title":    {"gonotch %s disponível", "gonotch %s is available"},
	"update_body":     {"Botão direito no notch › Atualizar, ou no terminal: gonotch update", "Right-click the notch › Update, or in a terminal: gonotch update"},
	"update_done":     {"gonotch atualizado para %s", "gonotch updated to %s"},
	"update_failed":   {"Não deu para atualizar o gonotch", "gonotch could not update"},
	"update_see_log":  {"O motivo está em: gonotch log", "The reason is in: gonotch log"},
	"updates":         {"Atualizações", "Updates"},
	"update_check":    {"Avisar sobre novas versões", "Tell me about new versions"},
	"update_hint":     {"Uma vez por dia pergunta ao GitHub qual é a última versão; nada além disso é enviado.", "Once a day, asks GitHub which version is the latest; nothing else is sent."},
